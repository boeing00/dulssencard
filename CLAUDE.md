...
- git: `main`, 원격 `boeing00/dulssencard` (**공개**). 2026-09-19 이력을 다시 써 강제 푸시했다(§11) — 그 전 SHA 는 전부 바뀌었다
...
1. [x] **공개 저장소 이력의 실제 개인정보** — §11. 2026-09-19 이력 재작성 · 강제 푸시 완료. GitHub 캐시 정리는 §11 끝.
...
## 11. 공개 저장소 이력에 남은 개인정보 (2026-09-13 발견 → 2026-09-19 이력 재작성)
...
작업 트리에서는 `b1a4b37`(재작성 전 SHA)에서 지웠다. 그 이전 커밋들이 `origin/main` 에 남아 있었다.
...
`git filter-repo --replace-text` 로 **모든 커밋 · 두 브랜치(`main`, `claude/dulssencard-bugs-improvements-lce1gd`)** 에서
실제 값을 지금 HEAD 의 익명 값으로 바꾸고 `--force-with-lease` 로 강제 푸시했다. 값 10종(실명 · 가린 이름 · 카드
뒷자리 2 · 가맹점 2 · 누적액 2 · 환전액 · 마스킹 카드번호)이 원격 전체 이력에서 0건인 것을 확인했다.
치환표에 원래 값을 적지 않는다 — 여기 적으면 다시 공개된다.
